...
<label class="field">Giá (₫)<input name="price" type="number" min="0" max="9999999999999999.99" step="0.01" value="<c:out value='${product.price}'/>" required></label>
<label class="field">Danh mục<select name="categoryId" required><option value="">Chọn danh mục</option>
<c:forEach items="${categories}" var="category"><option value="${category.id}" ${product.category.id == category.id ? 'selected' : ''}><c:out value="${category.name}"/></option></c:forEach>
</select></label>
