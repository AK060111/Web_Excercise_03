...
public class UserServiceImpl implements UserService {
    ...
    public boolean register(String username,String password,String email,String fullname,String phone){
        ...
        User user=new User(email,username,fullname,password,null,5,phone==null?"":phone,Date.valueOf(java.time.LocalDate.now()));
    ...
    public String verifyOtp(String username,String otp){
        if(otp==null || !otp.matches("[0-9]{6}")) return "FORMAT";
        User user=dao.get(username);
    ...
    public boolean resetPassword(String email,String grant,String password){
        if(grant==null || grant.length()<32 || password==null || password.isBlank() || password.length()>255) return false;
        User user=dao.getByEmail(email);
