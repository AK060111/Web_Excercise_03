...
public class ProductServiceImpl implements ProductService {
    ...
    public void validate(Product p){
        if(p==null||p.getName()==null||p.getName().isBlank())throw new IllegalArgumentException("Tên sản phẩm không được rỗng.");
        p.setName(p.getName().trim());
        if(p.getName().length()>255)throw new IllegalArgumentException("Tên sản phẩm tối đa 255 ký tự.");
        if(p.getPrice()==null||p.getPrice().signum()<0||p.getPrice().compareTo(new BigDecimal("9999999999999999.99"))>0)
            throw new IllegalArgumentException("Giá phải từ 0 đến 9999999999999999.99.");
