...
public class RegisterController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req,HttpServletResponse resp)throws ServletException,IOException {
        ...
        if(username.length()>100||password.length()>255||email.length()>255||fullname.length()>255||phone.length()>30){
            show(req,resp,"Thông tin nhập vượt quá độ dài cho phép");return;
        }
        try {new InternetAddress(email,true).validate();}
        catch(Exception e){show(req,resp,"Email không hợp lệ");return;}
        try {
