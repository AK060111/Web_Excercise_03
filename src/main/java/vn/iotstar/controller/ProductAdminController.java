...
public class ProductAdminController extends HttpServlet {
    ...
    public ProductAdminController(){this(new ProductServiceImpl(),new CategoryServiceImpl());}
    public ProductAdminController(ProductService service,CategoryService categories){this.service=service;this.categories=categories;}
    ...
    protected void doPost(HttpServletRequest req,HttpServletResponse resp)throws ServletException,IOException {
        ...
        boolean edit=path.endsWith("/edit");String uploaded=null;boolean saved=false;
        try {
            ...
            req.setAttribute("productPriceInput",req.getParameter("price"));req.setAttribute("productCategoryInput",req.getParameter("categoryId"));
            req.setAttribute("productSubmitted",true);
            try {input.setPrice(new BigDecimal(req.getParameter("price").trim()));Category c=new Category();c.setId(Integer.parseInt(req.getParameter("categoryId").trim()));input.setCategory(c);}
            ...
            if(!saved)ImageUpload.discardNewProductImage(uploaded);
            if(req.getAttribute("product")==null){resp.sendError(400,"ID sản phẩm không hợp lệ");return;}
            ((Product)req.getAttribute("product")).setImage(null);
            req.setAttribute("alert",e.getMessage());
            try{form(req,resp,edit);}catch(RuntimeException unavailable){resp.sendError(503,"Chưa thể tải biểu mẫu. Vui lòng thử lại sau.");}
        }catch(Exception e){if(!saved)ImageUpload.discardNewProductImage(uploaded);
            if(req.getAttribute("product")==null){resp.sendError(503,"Không thể lưu thay đổi. Vui lòng thử lại sau.");return;}
            ((Product)req.getAttribute("product")).setImage(null);
            req.setAttribute("alert","Không thể lưu thay đổi. Kiểm tra dữ liệu, ảnh tối đa 5 MB và thử lại.");
            try{form(req,resp,edit);}catch(RuntimeException unavailable){resp.sendError(503,"Chưa thể tải biểu mẫu. Vui lòng thử lại sau.");}}
    }
